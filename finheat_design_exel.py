...
L = .0025                   #Length of steel fins (m)
L2 = .0015                  #Length of steel wall (m)
...
t = .0013                   #Thickness of steel fins (m)
w = 1.0                         #Width of wall (m)
S = .013                    #Fin pitch(m)'''

''' import xlrd (code for giving an input in the excel)

book = xlrd.open_workbook("finheatdata.xls") 
 
sheet = book.sheets()[0] #book.sheets() returns a list of sheet objects... alternatively...

sheet = book.sheet_by_name("inputs") #we can pull by name
